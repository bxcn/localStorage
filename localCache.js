var CryptoJS = require( 'dir/third/crypto' );
/**
 * **********************************************************************
 ...
 *    indexKey 缓存名
 *    _cacheSize 缓存个数
 *    _localStorageSize 缓存数据的大小M单位
 *
 *   var localCache = new LRUCache(indexKey, _cacheSize, _localStorageSize);
 * 调用方式：
 ...
 * @constructor
 */
function LRUCache ( indexKey, _cacheSize, _localStorageSize ) {
  // 秒 分 时 一天
   ...
   * @param obj Object类型
   */
  function saveLocalStorage ( key, value, expires ) {
    expires  = expires || expiresTime;
    var time = +new Date() + expires * 60 * 1000; // 默认一天失效
    localStorage.setItem( key, value );
    saveKeyIndexLocalStorage( { key:key, time:time, size:getByteLen( value ) } );
  }
  ...
  function put ( key, value, expires ) {
    ...
    if ( item ) {
      ...
      if ( currentSize >= cacheSize ) {
        ...
        currentSize++;
        saveLocalStorage( key, value, expires );
      }
    }
  }
